...
export default function ManagementServiceCard({
  ...
  useEffect(() => {
    ...
    const promise = listenToEvent<InstallProgressEvent>(
      "management-install-progress",
      (ev) => {
        setPhase((current) => {
        ...
        const stepLabel =
          INSTALL_STEPS.find((s) => s.id === ev.step)?.label ?? ev.step;
        ...
        if (ev.status === "error") {
          ...
          appendLogRow(
            log.info("mgmt.install", `${stepLabel} started${detail}`, server.id),
          );
        } else if (ev.status === "ok") {
          appendLogRow(
            log.info("mgmt.install", `${stepLabel} ok${detail}`, server.id),
          );
        }
      },
    );
  ...
  const installedVersion =
    status.kind === "ok" ? status.value.installedVersion : null;
      ...
      appendLogRow(
        log.info(
          ...
          `${installed ? "Update" : "Install"} ${
            result.started
              ? "succeeded"
              : "completed but service not active"
          }${result.installedVersion ? ` (v${result.installedVersion})` : ""}.`,
          server.id,
        ),
      );
  ...
  const handleRestart = useCallback(async () => {
    ...
    appendLogRow(
      log.info(
        "mgmt.restart",
        `Restarting dune-server-service on ${server.host}…`,
        server.id,
      ),
    );
    ...
    try {
      ...
      appendLogRow(
        log.error("mgmt.restart", `Restart failed: ${message}`, server.id),
      );
  ...
  const handleUninstall = useCallback(async () => {
    ...
    appendLogRow(
      log.info(
        "mgmt.uninstall",
        `Uninstalling dune-server-service from ${server.host}…`,
        server.id,
      ),
    );
  ...
  const installedInProgress = phase.kind === "installing";
  ...
  return (
    <Card mt="3">
      ...
      <Dialog.Root
        open={installOpen}
        onOpenChange={(open) => {
          if (!open && !installedInProgress) closeInstall();
        }}
      >
        <Dialog.Content maxWidth="540px">
          <Dialog.Title>
            {installed ? "Update management service" : "Install management service"}
          </Dialog.Title>
          ...
          {phase.kind === "idle" ? (
            <Text size="2" color="gray">
              Ready to {installed ? "update" : "install"}.
            </Text>
          ) : (
            <Box>
      ...
      <AlertDialog.Root open={uninstallOpen} onOpenChange={setUninstallOpen}>
        <AlertDialog.Content maxWidth="420px">
          ...
          <AlertDialog.Description size="2">
            Stops and removes <Text className="mono">dune-server-service</Text> and its unit file.
